...
namespace hecate {
  ...
  public:
    ...
    void play_video_filtered(const string& in_video,
                             int step_sz=1,
                             int max_frm_len=360);
    /* Open video for getting frames*/
    int save_video_keyframes(const string &in_video, const string &out_dir, std::vector<int> &keyfrms);
    //bool get_video_frames_at_idx(const int &frm_idx, cv::Mat &img);
    //void close_video_frames();
    ...
    /* Get video frames */
    inline const vector<Mat> get_frm_rgb() {return _v_frm_rgb;}
    inline const double get_video_fps() {return _video_fps;}
